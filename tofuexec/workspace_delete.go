...
func (tf *OpenTofu) WorkspaceDelete(ctx context.Context, workspace string, opts ...WorkspaceDeleteCmdOption) error {
...
func (tf *OpenTofu) workspaceDeleteCmd(ctx context.Context, workspace string, opts ...WorkspaceDeleteCmdOption) (*exec.Cmd, error) {
	...
	for _, o := range opts {
		...
		case *LockOption, *LockTimeoutOption:
			...
			if err != nil {
				return nil, fmt.Errorf("-lock and -lock-timeout were added to workspace delete in OpenTofu 0.12: %w", err)
			}
		}
