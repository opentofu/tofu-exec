...
func (tf *OpenTofu) WorkspaceNew(ctx context.Context, workspace string, opts ...WorkspaceNewCmdOption) error {
...
func (tf *OpenTofu) workspaceNewCmd(ctx context.Context, workspace string, opts ...WorkspaceNewCmdOption) (*exec.Cmd, error) {
	...
	for _, o := range opts {
		...
		case *LockOption, *LockTimeoutOption:
			...
			if err != nil {
				return nil, fmt.Errorf("-lock and -lock-timeout were added to workspace new in OpenTofu 0.12: %w", err)
			}
		}
