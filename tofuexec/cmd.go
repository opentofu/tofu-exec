...
func (tf *OpenTofu) buildEnv(mergeEnv map[string]string) []string {
...
func (tf *OpenTofu) buildTofuCmd(ctx context.Context, mergeEnv map[string]string, args ...string) *exec.Cmd {
	cmd := exec.CommandContext(ctx, tf.execPath, args...)

	cmd.Env = tf.buildEnv(mergeEnv)
	cmd.Dir = tf.workingDir

	tf.logger.Printf("[INFO] running OpenTofu command: %s", cmd.String())

	return cmd
}

func (tf *OpenTofu) runTofuCmdJSON(ctx context.Context, cmd *exec.Cmd, v interface{}) error {
