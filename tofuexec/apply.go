...
func (tf *OpenTofu) Apply(ctx context.Context, opts ...ApplyOption) error {
...
func (tf *OpenTofu) ApplyJSON(ctx context.Context, w io.Writer, opts ...ApplyOption) error {
...
func (tf *OpenTofu) applyCmd(ctx context.Context, opts ...ApplyOption) (*exec.Cmd, error) {
...
func (tf *OpenTofu) applyJSONCmd(ctx context.Context, opts ...ApplyOption) (*exec.Cmd, error) {
	c := defaultApplyOptions
...
func (tf *OpenTofu) buildApplyArgs(ctx context.Context, c applyConfig) ([]string, error) {
	...
	if c.refreshOnly {
		...
		if err != nil {
			return nil, fmt.Errorf("refresh-only option was introduced in OpenTofu 0.15.4: %w", err)
		}
		if !c.refresh {
	...
	if c.replaceAddrs != nil {
		...
		if err != nil {
			return nil, fmt.Errorf("replace option was introduced in OpenTofu 0.15.2: %w", err)
		}
	...
	if c.destroy {
		err := tf.compatible(ctx, tf0_15_2, nil)
		if err != nil {
			return nil, fmt.Errorf("-destroy option was introduced in OpenTofu 0.15.2: %w", err)
		}
...
func (tf *OpenTofu) buildApplyCmd(ctx context.Context, c applyConfig, args []string) (*exec.Cmd, error) {
