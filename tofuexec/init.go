...
func (tf *OpenTofu) Init(ctx context.Context, opts ...InitOption) error {
...
func (tf *OpenTofu) initCmd(ctx context.Context, opts ...InitOption) (*exec.Cmd, error) {
	...
	for _, o := range opts {
		...
		case *LockOption, *LockTimeoutOption, *VerifyPluginsOption, *GetPluginsOption:
			...
			if err != nil {
				return nil, fmt.Errorf("-lock, -lock-timeout, -verify-plugins, and -get-plugins options are no longer available as of OpenTofu 0.15: %w", err)
			}
		}
