...
func (tf *OpenTofu) RefreshJSON(ctx context.Context, w io.Writer, opts ...RefreshCmdOption) error {
	err := tf.compatible(ctx, tf0_15_3, nil)
	if err != nil {
		return fmt.Errorf("tofu refresh -json was added in 0.15.3: %w", err)
	}
...
func (tf *OpenTofu) refreshJSONCmd(ctx context.Context, opts ...RefreshCmdOption) (*exec.Cmd, error) {
	c := defaultRefreshOptions
...
func (tf *OpenTofu) buildRefreshArgs(c refreshConfig) []string {
...
func (tf *OpenTofu) buildRefreshCmd(ctx context.Context, c refreshConfig, args []string) (*exec.Cmd, error) {
