...
func (tf *OpenTofu) Graph(ctx context.Context, opts ...GraphOption) (string, error) {
...
func (tf *OpenTofu) graphCmd(ctx context.Context, opts ...GraphOption) (*exec.Cmd, error) {
	...
	if c.plan != "" {
		// plan was a positional arguement prior to OpenTofu 0.15.0. Ensure proper use by checking version.
	...
	if c.drawCycles {
		...
		if err != nil {
			return nil, fmt.Errorf("-draw-cycles was first introduced in OpenTofu 0.5.0: %w", err)
		}
	...
	if c.graphType != "" {
		...
		if err != nil {
			return nil, fmt.Errorf("-graph-type was first introduced in OpenTofu 0.8.0: %w", err)
		}
