...
func (tf *OpenTofu) Taint(ctx context.Context, address string, opts ...TaintOption) error {
	err := tf.compatible(ctx, tf0_4_1, nil)
	if err != nil {
		return fmt.Errorf("taint was first introduced in OpenTofu 0.4.1: %w", err)
	}
	taintCmd := tf.taintCmd(ctx, address, opts...)
	return tf.runTofuCmd(ctx, taintCmd)
}

func (tf *OpenTofu) taintCmd(ctx context.Context, address string, opts ...TaintOption) *exec.Cmd {
