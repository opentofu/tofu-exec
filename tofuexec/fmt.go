...
func FormatString(ctx context.Context, execPath string, content string) (string, error) {
	tf, err := NewOpenTofu(filepath.Dir(execPath), execPath)
	if err != nil {
		return "", err
	}
...
func (tf *OpenTofu) FormatString(ctx context.Context, content string) (string, error) {
...
func (tf *OpenTofu) Format(ctx context.Context, unformatted io.Reader, formatted io.Writer) error {
...
func (tf *OpenTofu) FormatWrite(ctx context.Context, opts ...FormatOption) error {
	for _, o := range opts {
		switch o := o.(type) {
		case *DirOption:
			if o.path == "-" {
...
func (tf *OpenTofu) FormatCheck(ctx context.Context, opts ...FormatOption) (bool, []string, error) {
	for _, o := range opts {
		switch o := o.(type) {
		case *DirOption:
			if o.path == "-" {
...
func (tf *OpenTofu) formatCmd(ctx context.Context, args []string, opts ...FormatOption) (*exec.Cmd, error) {
	err := tf.compatible(ctx, tf0_7_7, nil)
	if err != nil {
		return nil, fmt.Errorf("fmt was first introduced in OpenTofu 0.7.7: %w", err)
	}
	...
	for _, o := range opts {
		...
		case *RecursiveOption:
			...
			if err != nil {
				return nil, fmt.Errorf("-recursive was added to fmt in OpenTofu 0.12: %w", err)
			}
		}
