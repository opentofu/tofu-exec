...
func TestUnparsedError(t *testing.T) {
	...
	runTest(t, "", func(t *testing.T, tfv *version.Version, tf *tofuexec.OpenTofu) {
...
func TestMissingVar(t *testing.T) {
	runTest(t, "var", func(t *testing.T, tfv *version.Version, tf *tofuexec.OpenTofu) {
		err := tf.Init(context.Background())
		if err != nil {
		...
		_, err = tf.Plan(context.Background(), tofuexec.Var(longVarName+"=foo"))
		if err == nil {
			t.Fatalf("expected error running Plan, none returned")
		}

		_, err = tf.Plan(context.Background(), tofuexec.Var(shortVarName+"=foo"))
		if err == nil {
			t.Fatalf("expected error running Plan, none returned")
		}
		...
		_, err = tf.Plan(context.Background(), tofuexec.Var(shortVarName+"=foo"), tofuexec.Var(longVarName+"=foo"))
		if err != nil {
...
func TestTFVersionMismatch(t *testing.T) {
	runTest(t, "tf99", func(t *testing.T, tfv *version.Version, tf *tofuexec.OpenTofu) {
...
func TestLockedState(t *testing.T) {
	runTest(t, "inmem_backend_locked", func(t *testing.T, tfv *version.Version, tf *tofuexec.OpenTofu) {
		err := tf.Init(context.Background())
		if err != nil {
			t.Fatalf("err during init: %s", err)
		}
...
func TestContext_sleepNoCancellation(t *testing.T) {
	...
	runTest(t, "sleep", func(t *testing.T, tfv *version.Version, tf *tofuexec.OpenTofu) {
		// only testing versions that can cancel mid apply
		if !tfv.GreaterThanOrEqual(protocol5MinVersion) {
			t.Skip("the ability to interrupt an apply was added in protocol 5.0 in OpenTofu 0.12, so test is not valid")
		}
		...
		start := time.Now()
		err = tf.Apply(ctx, tofuexec.Var(`create_duration=5s`))
		if err != nil {
