...
func TestValidate(t *testing.T) {
	runTest(t, "basic", func(t *testing.T, tfv *version.Version, tf *tofuexec.OpenTofu) {
		if tfv.LessThan(validateMinVersion) {
			t.Skip("tofu validate -json was added in Terraform 0.12, so test is not valid")
		}
	...
	runTest(t, "invalid", func(t *testing.T, tfv *version.Version, tf *tofuexec.OpenTofu) {
		if tfv.LessThan(validateMinVersion) {
			t.Skip("tofu validate -json was added in Terraform 0.12, so test is not valid")
		}
		...
		if err != nil {
			...
			// unsure why 0.12 tofu init does not have a non-zero exit code for syntax problems
			if err == nil {
