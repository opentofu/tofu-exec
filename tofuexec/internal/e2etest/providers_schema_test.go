...
var (
	v1_6 = version.Must(version.NewVersion("1.6.0"))
	v1_7 = version.Must(version.NewVersion("1.7.0"))
	v1_8 = version.Must(version.NewVersion("1.8.0"))
	v1_9 = version.Must(version.NewVersion("1.9.0"))
)
							...
							Block: &tfjson.SchemaBlock{
								...
								Attributes: map[string]*tfjson.SchemaAttribute{
									...
									"random": {
										AttributeType: cty.String,
										Computed:      true,
										Description:   "A random value. This is primarily for testing and has little practical use; prefer the [hashicorp/random provider](https://search.opentofu.org/provider/hashicorp/random/latest) for more practical random number use-cases.",
									},
								},
							},
						},
...
`,
		...
		t.Run(fmt.Sprintf("%d %s", i, c.fixtureDir), func(t *testing.T) {
			runTest(t, c.fixtureDir, func(t *testing.T, tfv *version.Version, tf *tofuexec.OpenTofu) {
