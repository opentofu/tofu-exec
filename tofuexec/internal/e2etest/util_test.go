...
func runTest(t *testing.T, fixtureName string, cb func(t *testing.T, tfVersion *version.Version, tf *tofuexec.OpenTofu)) {
	t.Helper()
	...
	if override := os.Getenv("TOFUEXEC_E2ETEST_VERSIONS"); override != "" {
		versions = strings.Split(override, ",")
	}

	// If the env var TOFUEXEC_E2ETEST_TOFU_PATH is set to the path of a
	// valid OpenTofu executable, only tests appropriate to that
	// executable's version will be run.
	if localBinPath := os.Getenv("TOFUEXEC_E2ETEST_TOFU_PATH"); localBinPath != "" {
		// By convention, every new OpenTofu struct is given a clean
		...
		ltf, err := tofuexec.NewOpenTofu(td, localBinPath)
		if err != nil {
			t.Fatal(err)
		}

		ltf.SetAppendUserAgent("tofuexec-e2etest")

		lVersion, _, err := ltf.Version(context.Background(), false)
		if err != nil {
			t.Fatalf("unable to determine version of OpenTofu binary at %s: %s", localBinPath, err)
		}
...
func runTestWithVersions(t *testing.T, versions []string, fixtureName string, cb func(t *testing.T, tfVersion *version.Version, tf *tofuexec.OpenTofu)) {
	t.Helper()
		...
		t.Run(fmt.Sprintf("%s-%s", fixtureName, tfv), func(t *testing.T) {
			if !strings.HasPrefix(tfv, "refs/") && runtime.GOOS == "darwin" && runtime.GOARCH == "arm64" {
				...
				if v.LessThan(version.Must(version.NewVersion("1.0.2"))) {
					t.Skipf("OpenTofu not available for darwin/arm64 < 1.0.2 (%s)", v)
				}
			}
			...
			tf, err := tofuexec.NewOpenTofu(td, execPath)
			if err != nil {
				t.Fatal(err)
			}

			tf.SetAppendUserAgent("tofuexec-e2etest")
			...
			// skipped when using a local OpenTofu executable.
