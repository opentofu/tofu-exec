...
// Functions is the top-level object returned when exporting function
// signatures
...
// Validate checks to ensure that Functions is present, and the
...
func (f *Functions) Validate() error {
	...
	v, err := version.NewVersion(f.FormatVersion)
	if err != nil {
...
// UnmarshalJSON implements json.Unmarshaler for Functions.
//
// As per established convention this method should only ever
// be invoked *indirectly* via [encoding/json] library.
func (f *Functions) UnmarshalJSON(b []byte) error {
	type rawFunctions Functions
